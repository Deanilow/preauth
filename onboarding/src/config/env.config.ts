...
export const validateEnvironment = (): void => {
  const issues: EnvironmentValidationIssue[] = REQUIRED_ENV_VARS
    ...
    .map((name) => ({ name, reason: `required - [sur-auth-ms-onboarding-onboarding-orchestrator]` }));

  if (issues.length > 0) {
    const missingVars = issues.map((i) => `  - ${i.name}`).join('\n');
    const error = new Error(
      `Invalid environment configuration - [sur-auth-ms-onboarding-onboarding-orchestrator]\n` +
      `Missing variables:\n${missingVars}`
