...
export const BUSINESS_ERROR_CODES = {
  ...
  // ── Autenticación / tokens (contextToken, sessionToken) ──
  MISSING_AUTH_TOKEN: { code: 'ONB-AUTH-001', status: 401, message: 'El token de autorización es obligatorio.' },
  INVALID_TOKEN: { code: 'ONB-AUTH-002', status: 401, message: 'El token es inválido, expiró o no corresponde a este flujo.' },
  TOKEN_REPLAY: { code: 'ONB-AUTH-003', status: 401, message: 'El token ya fue utilizado (replay detectado).' },
  REQUEST_REPLAYED: { code: 'ONB-AUTH-004', status: 409, message: 'La solicitud ya fue procesada (X-Request-Id duplicado).' },
  CHANNEL_MISMATCH: { code: 'ONB-AUTH-005', status: 403, message: 'El canal del token no coincide con el de la sesión.' },
  IP_BINDING_MISMATCH: { code: 'ONB-AUTH-006', status: 403, message: 'La IP del cliente no coincide con la IP vinculada al token.' },
  MISSING_REQUEST_ID: { code: 'ONB-AUTH-007', status: 400, message: 'El header X-Request-Id es obligatorio.' },

  // ── State machine de onboarding ──
  INVALID_STEP_SEQUENCE: { code: 'ONB-BNS-001', status: 409, message: 'La sesión no se encuentra en el paso esperado.' },
  STEP_EXPIRED: { code: 'ONB-BNS-002', status: 410, message: 'El paso actual expiró, reinicie el flujo.' },

  // ── Validaciones de negocio por paso ──
  INVALID_DNI: { code: 'ONB-BNS-010', status: 422, message: 'El DNI ingresado no es válido.' },
  INVALID_OTP: { code: 'ONB-BNS-011', status: 422, message: 'El código OTP ingresado es incorrecto.' },
  FACE_VALIDATION_FAILED: { code: 'ONB-BNS-012', status: 422, message: 'La validación biométrica facial falló.' },
  OCR_VALIDATION_FAILED: { code: 'ONB-BNS-016', status: 422, message: 'La validación del documento (OCR) falló.' },
  INVALID_PRODUCTS_SELECTION: { code: 'ONB-BNS-013', status: 422, message: 'Debe seleccionar al menos un producto.' },
  INVALID_PASSWORD: { code: 'ONB-BNS-014', status: 422, message: 'La contraseña no cumple con los requisitos mínimos.' },
  DEVICE_ENROLLMENT_FAILED: { code: 'ONB-BNS-015', status: 502, message: 'No se pudo enrolar el dispositivo.' },
} as const;
...
export const SERVICE_UNAVAILABLE_CODE_MAP: Record<string, string> = {
  TokenService: 'TOK-UNV-001',
  SessionService: 'SES-UNV-001',
  DeviceEnrollmentService: 'DEV-UNV-001',
};
...
const PROVIDER_FALLBACK_CODE: Record<string, string> = {
  TOKEN_SERVICE_API: 'TOK-BIZ-001',
  SESSION_SERVICE_API: 'SES-BIZ-001',
  DEVICE_ENROLLMENT_API: 'DEV-BIZ-001',
};
...
const SOURCE_SYSTEM_MAP: Record<string, string> = {
  ...
  DeviceEnrollmentService: 'DEVICE_ENROLLMENT_API',
};
